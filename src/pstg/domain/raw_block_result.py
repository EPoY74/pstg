...
class RawBlockResult:
    ...
    # (например Modbus TCP -> Modbus RTU), чтобы выбрать
    # RTU-устройство за шлюзом.
    unit_id: int = 0

    # ok (Успех блока):
    # True  -> блок прочитан/записан успешно, registers заполнен.
    # False -> блок не получен (ошибка устройства или транспорта),
    #  registers НЕ использовать.
    # None сейчас допускается, но best practice: сделать просто
    # bool и всегда заполнять.
    ok: bool | None = None
    ...
    # - kind="device" (ошибка устройства):
    # Modbus exception (Illegal Function / Illegal Data Address и т.д.).
    # Если ok=True -> error обычно None.
    current_error_info: ErrorInfo | None = None

    # duration_ms (Длительность, миллисекунды):
    # Сколько заняло это чтение "от отправки запроса
    # до получения ответа" (latency).
