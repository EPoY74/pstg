...
async def open_connection_modbus_tcp(
    ...
    """Создает соединение с TCP Modbus контроллером, автоматически
    переподключается.
    Args:
        ip_host (str): IP адрес устройства или хоста, за которым устройство
        ip_port (int, optional): Номер порта, который работает с modbus.
        Defaults to 502.
    Raises:
        ConnectionException: Failed to connect to Modbus server (modbus)
        ConnectionError: Ошибка подключения к modbus серверу (общая)
        ConnectionError: Ошибка подключения к modbus
        серверу (генерируется клиентом)
    Returns:
    ...
    #  reconnect_delay автоматически удваивается при каждой
    ...
    try:
        ...
        raise ConnectionError(
            f"Failed to connect to Modbus server {ip_host}:{ip_port}"
        ) from err
