...
- поднять стабильный polling `Modbus TCP`
- читать данные с устройства
- различать `device error` и `transport error`
- понимать, жив канал или нет
- иметь локальный simulator для разработки без реального оборудования
...
- хранить telemetry batches по run id
...
Важно: `ok` не равен состоянию канала.  
Канал оценивается через `PollResult.connection_state`.

#### `registers: list[int]`

Сырые значения Modbus регистров.

Это `raw telemetry data`, из которых потом можно строить:
